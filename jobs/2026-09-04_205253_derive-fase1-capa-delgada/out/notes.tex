...
\src{espectro_dd,expansion_modal}
...
tasa más lenta.
\src{espectro_dn,expansion_modal}
...
Para el espesor motivador \(h=6\times10^{-3}\,\mathrm m\), la segunda condición
de~\eqref{eq:desigualdades} equivale a
...
El trabajo no contiene una velocidad típica \(U\) ni la menor escala horizontal
experimental \(\ell\). Por eso puede darse la predicción paramétrica
\eqref{eq:h6}, pero no decidir con la información disponible si la clausura de
un modo es aplicable a la celda concreta. Esa decisión requiere ambos datos.
\src{escalamiento_validez,aplicabilidad_experimental}
...
\end{equation}
calculando \(\sinh(kz)/k=z\,\mathrm{sinhc}(kz)\). Así se evalúan directamente
el caso exacto \(k=0\) y la rama que la implementación selecciona cuando
\(kL_z<0.5\); la base exponencial acotada cubre el régimen grande solicitado.
El umbral basado sólo en \(kL_z\) no constituye, sin embargo, una garantía
uniforme frente a escalas arbitrariamente dispares de \(g_0\) y \(g_1\): en la
rama exponencial aún pueden cancelarse los términos proporcionales a \(g_1/k\).
\src{limite_k0,limitacion_k_pequeno}
...
\section{Salvedades y puntos abiertos}
...
\item La estabilidad para \(kL_z\) grande sí fue comprobada, pero no hay una
  garantía uniforme para todo \(k>0\) y toda escala de datos de borde. En el
  contraejemplo \(k=5\times10^{-11}\), \(L_z=10^{10}\), \(g_0=1\) y \(g_1=7.3\),
  que cae justo en la rama exponencial con \(kL_z=0.5\), se obtuvo
  \(\lvert\phi(0)-g_0\rvert=7.629\times10^{-6}\). Esto refuta la promesa más
  amplia de ausencia de cancelación, no la estabilidad grande requerida por el
  contrato. \src{limitacion_k_pequeno}
\item \texttt{capa.py} restringe la evaluación al intervalo físico
  \(0\le z\le L_z\). La malla FC--Gram de SPECTER también contiene puntos de
  continuación con \(z>L_z\); el comportamiento de esta rama allí no fue
  verificado y queda para la fase de integración Fortran. \src{fc_gram_pendiente}
\item El primer control espectral de la puerta pasó con error
  \(8.23\times10^{-5}\) frente a una tolerancia \(10^{-4}\): el margen es sólo
  un factor aproximado de \(1.2\). El verifier reprodujo ese valor mediante el
  error de truncamiento de la referencia FD de la propia puerta; aun así, no se
  comprobó una variante con índices modales mayores y el margen no debe
  extrapolarse a ella. \src{margen_puerta}
